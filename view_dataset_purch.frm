TYPE=VIEW
query=select distinct `a`.`no_purchase` AS `no_purchase`,`a`.`id_supplier` AS `id_supplier`,concat((case when isnull(`b`.`first_name`) then \'\' else `b`.`first_name` end),\' \',(case when isnull(`b`.`last_name`) then \'\' else `b`.`last_name` end)) AS `supplier_name`,`b`.`address` AS `address`,`a`.`date_trn` AS `date_trn` from (((`ideaccounting_dev`.`trn_purchase_head` `a` join `ideaccounting_dev`.`trn_purchase_det` `s` on((`s`.`no_purchase` = `a`.`no_purchase`))) join `ideaccounting_dev`.`mst_supplier` `b` on((`b`.`id_supplier` = `a`.`id_supplier`))) left join `ideaccounting_dev`.`view_total_purchase_return` `f` on(((`f`.`id_item` = `s`.`id_item`) and (`f`.`no_purchase` = `s`.`no_purchase`)))) where ((case when isnull(`f`.`qty`) then 0 else `f`.`qty` end) < (case when isnull(`s`.`qty`) then 0 else `s`.`qty` end))
md5=56d005b348b650c7446acf80302f9599
updatable=0
algorithm=0
definer_user=root
